...
void controller_expose(gx_guvnorUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
void resize_event(gx_guvnorUI *ui) {
...
bool get_active_ctl_num(gx_guvnorUI *ui, int *num) {
	static bool ret;
	ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
void scroll_event(gx_guvnorUI *ui, int direction) {
	float value;
	int num;
...
void button1_event(gx_guvnorUI *ui, double* start_value) {
	int num;
	if (get_active_ctl_num(ui, &num)) {
...
void motion_event(gx_guvnorUI *ui, double start_value, int m_y) {
...
void set_key_value(gx_guvnorUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
void key_event(gx_guvnorUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
void set_previous_controller_active(gx_guvnorUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
void set_next_controller_active(gx_guvnorUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
