...
#define GTK_GUV_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_GUV_KNOB, GtkGuvKnobPrivate))

G_DEFINE_TYPE(GtkGuvKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkGuvKnobPrivate GtkGuvKnobPrivate;

struct _GtkGuvKnobPrivate
{
	int knob_x;
	int knob_y;
	int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
	/** check resize **/
	int grow;
 	GtkGuvKnobPrivate *priv = GTK_GUV_KNOB_GET_PRIVATE(widget);
...
{
	g_assert(GTK_IS_GUV_KNOB(widget));
	GtkGuvKnob *knob = GTK_GUV_KNOB(widget);
	GtkGuvKnobPrivate *priv = GTK_GUV_KNOB_GET_PRIVATE(knob);
...
/****************************************************************
 ** init the GtkGuvKnobClass
 */

static void gtk_knob_class_init (GtkGuvKnobClass *klass)
{
...
//--------- connect the events with funktions
	...
	g_type_class_add_private(obj_class, sizeof (GtkGuvKnobPrivate));
}
...
static void gtk_knob_init (GtkGuvKnob *knob)
{
...
{
	GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_GUV_KNOB, NULL ));
