...
static void resize_event(gx_guvnorUI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_guvnorUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
	...
	gx_guvnorUI* ui = (gx_guvnorUI*)malloc(sizeof(gx_guvnorUI));
	...
	ui->controls[0] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 50, 60, 61, 61}, false,"LEVEL", KNOB, LEVEL};
	ui->controls[1] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 120, 30, 61, 61}, false,"BASS", KNOB, BASS};
	ui->controls[2] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 190, 25, 61, 61}, false,"MID", KNOB, MID};
	ui->controls[3] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 260, 30, 61, 61}, false,"TREBLE", KNOB, TREBLE};
	ui->controls[4] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 330, 60, 61, 61}, false,"GAIN", KNOB, GAIN};
	ui->block_event = -1;
...
static void cleanup(LV2UI_Handle handle) {
	gx_guvnorUI* ui = (gx_guvnorUI*)handle;
	cairo_destroy(ui->cr);
...
static void knob_expose(gx_guvnorUI *ui,gx_controller* knob) {
	...
	if (knob->is_active) {
		cairo_set_source_rgba (ui->crf, 1.0, 1.0, 1.0, 1.0);
	} else {
...
static void bypass_expose(gx_guvnorUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_guvnorUI *ui, gx_controller* controller) {
...
static void _expose(gx_guvnorUI *ui) {
	static const char* plug_name = "GxGuvnor" ;
...
static void controller_expose(gx_guvnorUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
static void send_controller_event(gx_guvnorUI *ui, int controller) {
...
static void check_value_changed(gx_guvnorUI *ui, int i, float* value) {
...
static void check_is_active(gx_guvnorUI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_guvnorUI *ui) {
...
static bool get_active_ctl_num(gx_guvnorUI *ui, int *num) {
	static bool ret;
	ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num(gx_guvnorUI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_guvnorUI *ui, int direction) {
	float value;
	int num;
...
static void switch_event(gx_guvnorUI *ui, int i) {
...
static void button1_event(gx_guvnorUI *ui, double* start_value) {
	int num;
	if (get_active_ctl_num(ui, &num)) {
...
static void motion_event(gx_guvnorUI *ui, double start_value, int m_y) {
...
static void set_key_value(gx_guvnorUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_guvnorUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_guvnorUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_guvnorUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
void get_last_active_controller(gx_guvnorUI *ui, bool set) {
...
static void event_handler(gx_guvnorUI *ui) {
	XEvent xev;
