...
G_DEFINE_TYPE(GxGuvPaintBox, gx_paint_box, GTK_TYPE_BOX)

static void gx_paint_box_class_init (GxGuvPaintBoxClass *klass)
{
...
static void set_paint_func(GxGuvPaintBox *paint_box, const gchar *paint_func)
{
	gchar *spf;
...
static void gx_paint_box_style_set(GtkWidget *widget, GtkStyle  *previous_style)
{
	GxGuvPaintBox *paint_box = GX_GUV_PAINT_BOX(widget);
	set_paint_func(paint_box, paint_box->paint_func);
}


static void gx_paint_box_init (GxGuvPaintBox *paint_box)
{
...
static void gx_paint_box_destroy(GtkObject *object)
{
	GxGuvPaintBox *paint_box = GX_GUV_PAINT_BOX(object);
	...
	GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
	switch (prop_id) {
	case PROP_PAINT_FUNC:
		g_value_set_string(value, GX_GUV_PAINT_BOX(object)->paint_func);
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
		break;
...
static void pedal_expose(GtkWidget *wi, GdkEventExpose *ev)
{
	...
	cairo_t *cr;
	GxGuvPaintBox *paintbox = GX_GUV_PAINT_BOX(wi);
